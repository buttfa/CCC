 ...
 * @return int 
 */
int isPathInList(const struct file_node* node, const char* path) {
    while (node != NULL) {
 ...
 * @param path 
 */
void appendToList(struct file_node** head, const char* path) {
    ...
    if ((dir = opendir(sll_folder_path)) != NULL) {
        while ((ent = readdir(dir)) != NULL) {
            ...
            if (ent->d_type == DT_DIR) {
                ...
                // 检查并添加.a文件到链表
                if (!isPathInList(sll_list, full_path)) {
                    appendToList(&sll_list, full_path);
                }
            }
        }
        closedir(dir);
