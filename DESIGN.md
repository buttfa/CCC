...
## 工作原理
#### （一）获得头文件目录
##### 1. 遍历$(header_folder_path)和$(extern_library_path)中的文件夹，找到包含.h文件的文件夹，确认路径不重复后为其创造header_folder结构体，并添加到header_folder_list链表中。（为了确保路径不重复，每次添加前都需要从头遍历head_folder_list链表，确认该路径不存在后才添加节点）
...
#### （二）计算依赖关系
##### 中间文件的依赖关系
##### 1. 遍历$(source_folder_path)和$(extern_library_path)，找到.c/.cpp文件。
...
#### （四）载入静态链接库文件
##### 1. 遍历(sll_folder_path)和(extern_library_path)，找到.a文件。
...
#### （五）载入动态链接库文件
##### 1. 遍历$(dll_folder_path)和$(extern_library_path)，找到.so文件。
